...
| **Negative cache** | An in-memory `Set<string>` of app names whose icon resolution failed during this session. Skipped on re-attempts until the next process restart. Pairs with the 30-day TTL: TTL governs _how often_ we re-resolve; the negative cache prevents _retry storms_ within a single session when osascript is failing.                                                        |
