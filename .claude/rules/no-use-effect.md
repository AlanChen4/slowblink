...
**Vendored shadcn/ui primitives in `src/components/ui/` are exempt.** These files are copied verbatim from shadcn and intentionally tracked against upstream — refactoring their `useEffect` calls would diverge them and complicate future shadcn updates. Example: `src/components/ui/carousel.tsx`. Wrappers and feature components built _on top of_ these primitives are not exempt and must follow the rule.
...
```typescript
// Located at src/hooks/use-mount-effect.ts
